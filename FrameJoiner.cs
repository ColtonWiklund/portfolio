...
// When a new Frame Segment/Panel is created that connects to this joiner, it must be added to the respective list to show it is connected
		...
		namespace Frame
		{
			public class FrameJoiner : MonoBehaviour {
				...
				public void AddJoiner (GameObject frameSegment) {
					...
					//Debug.Log ("--- " + this.gameObject.name + " has connected Frame Joiners: ---");
					//foreach (GameObject joiner in connectedJoiners) {
					//	Debug.Log (joiner.name);
					//}
				}
